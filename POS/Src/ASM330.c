...
/*********************************************************************************************************
**	函数原型:	u8 ASM330_ParityBit(u32 data)
...
uint8_t ASM330_Init(void)
{
	...
	SPI_Write(data, 4);
    HAL_Delay(100);
    uint16_t PID = ASM330_GetRegisterValue(ASM330_PID1);
	if((PID >> 8) != 0x52)
	{
		status = 0x0;
	}
...
/*********************************************************************************************************
**	函数原型:	u16 ASM330_GetRegisterValue(u8 regAddress)
**	函数功能:	读取陀螺仪寄存器的值
**	输入参数:	无
**	输出参数:	无
**	返回参数:	读取的陀螺仪寄存器值
...
uint16_t ASM330_GetRegisterValue(uint8_t regAddress)
{
	...
	data[0] = ASM330_READ_DATA | (regAddress >> 7);
	data[1] = regAddress << 1;
	dataToSend += ((uint32_t)data[0] << 24);
	dataToSend += ((uint32_t)data[1] << 16);
	data[3] = ASM330_ParityBit(dataToSend);
	SPI_Write(data, 4);
...
/*********************************************************************************************************
**	函数原型:	void ASM330_SetRegisterValue(u8 regAddress, u16 regData)
**	函数功能:	设置陀螺仪寄存器的值
**	输入参数:	无
**	输出参数:	无
**	返回参数:	无
...
/*********************************************************************************************************
**	函数原型:	u8 ASM330_GetTemperature(void)
**	函数功能:	读出陀螺仪的温度
**	输入参数:	无
**	输出参数:	无
**	返回参数:	温度
...
uint8_t ASM330_GetTemperature(void)
{
	...
	temp = ASM330_GetRegisterValue(ASM330_TEMP1);
	temp >>= 6;	
	temp -= 0x31f;
	temp /= 5;
